...
import org.hhplus.lecture.Helper.createLecture
import org.hhplus.lecture.Helper.createLectureSession
...
class LectureSessionControllerTest @Autowired constructor(
    ...
    inner class ApplicationTest {
        ...
        fun `should return 200 ok with an enrolled lecture session`() {
            // given
            val periodFrom = LocalDateTime.now().minusDays(1) // 1일 전부터 5일 동안 신청 기한
            val scheduleFrom = LocalDateTime.now().plusWeeks(1) // 1주일 뒤부터 1시간 동안 강의 시간
         ...
         * 기한이 맞지 않아 실패했다는 error response 를 내려줘야 한다.
         * */
        @Test
         ...
         * 정원이 초과했다는 error response 를 내려줘야 한다.
         * */
        @Test
         ...
         * 중복 신청했다는 error response를 내려줘야 한다.
         * */
        @Test
        fun `should return 400 bad request when trying to enroll in the already booked lecture`() {
            ...
            val periodFrom = LocalDateTime.now().minusDays(1) // 1일 전부터 5일 동안 신청 기한
            val scheduleFrom = LocalDateTime.now().plusWeeks(1) // 1 주일 뒤 부터 1시간 동안 강의 시간
            val lectureSession = lectureSessionRepository.save(createLectureSession(lecture, periodFrom, scheduleFrom))
            enrollmentRepository.save(Enrollment(1L, student, lectureSession, LocalDateTime.now().minusMinutes(1)))
